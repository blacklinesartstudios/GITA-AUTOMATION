...
def get_authenticated_service(project_root: Path):
    ...
    Loads token.json if present; if expired, refreshes seamlessly via Request().
    Only opens browser for interactive OAuth consent if no valid token exists.
    """
    ...
    if not client_secrets_path.exists():
        # Fallback check for alternate secret naming
...
def format_shorts_metadata(chapter: int, verse: int, sanskrit: str, meaning: str, insight: str, music_attribution: str = "") -> dict:
    """
    Constructs high-engagement, human-crafted title, description, and tags for YouTube Shorts.
    """
    # Dynamic Title
...
def upload_short_to_youtube(
    ...
    media = MediaFileUpload(
        str(video_path),
        chunksize=1024 * 1024 * 8,  # 8MB chunk buffer
        resumable=True,
        mimetype="video/mp4"
    )
